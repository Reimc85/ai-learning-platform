...
def create_app():
    # Configure Flask to serve static files from the frontend/dist directory
    ...
    # Database initialization (temporarily commented out db.drop_all and db.create_all for debugging)
    with app.app_context():
        try:
            ...
            pass # Keep pass here as db.drop_all/create_all are commented out
            print("Database initialization complete (skipped table creation).", file=sys.stderr) # Updated message
        except Exception as e:
            print(f"ERROR: Database initialization failed: {e}", file=sys.stderr)
            raise e # Keep raise e to catch any other DB init issues

    @app.route('/api/health', methods=['GET'])
    def health_check():
        """Health check endpoint"""
        return jsonify({ # Use jsonify for API responses
            'status': 'healthy',
    ...
    def serve(path):
        ...
        if path.startswith('api/'):
            # If it's an API route, let the blueprints handle it, or return 404 if not found by blueprints
            # This specific return is for paths like /api/something that don't match a blueprint
        ...
        if path != "" and os.path.exists(requested_file_path) and not os.path.isdir(requested_file_path):
            return send_from_directory(static_folder_path, path)
        else:
            # Fallback to index.html for all other paths (React Router handles client-side routing)
